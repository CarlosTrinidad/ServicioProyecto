...
echo "<h3>Horario Salones<h3>";

$this->title = Yii::t('app', 'Horario por salones');
...
foreach ($rooms as $room) {
	#Se inicializa el horario
	$horario=setSchedule($interval);
	# code...
	echo "<br><h4>".$room->room."<br></h4>";
...
for($i=0;$i<$f;$i++){
	...
	for($j=0;$j<$c;$j++){
		if($i==0 or $j==0){
	        ...
	        echo $matrix[$i][$j];
	        // echo $matrix[$i][0].":00";
					echo Html::a('<span class="glyphicon glyphicon-plus"></span>', ['classes/create','room' => $room->id, 'day' => $j, 'time_start' => $matrix[$i][0].":00",'return' => 'yes' ]);
				echo "</td>";}
            } }
	}
	echo "</tr>";
